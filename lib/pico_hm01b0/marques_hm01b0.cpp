// See: https://github.com/pmarques-dev/PicoHM01B0/blob/main/src/PicoHM01B0.cc
...
public:
	...
	uint start_capture(uint8_t *dest);
	...
	int get_cols(void) const {
		return binning_2x2 ? 160 : 320;
		// return binning_2x2 ? 164 : 324;  // original
	}
	int get_rows(void) const {
		int ret = qvga_mode ? 240 : 320;
		// int ret = qvga_mode ? 244 : 324;  // original
...
private:
	...
	// the camera code only uses i2c for initialization, so we just do a
	...
	int image_program_init(uint *offset);
...
int PicoHM01B0::image_program_init(uint *offset)
{
	int bus_bits = config.bus_4bit ? 4 : 1;
	int num_pclk_per_px = config.bus_4bit ? 2 : 8;

	// THE FOLLLOWING IS ORIGINAL, BUT INCLUDES 2 PIXEL BORDER AROUND IMAGE
	...
	// PIO SM INSTRUCTIONS "BORROWED" FROM ADE HM01B0
	constexpr uint8_t num_border_px = 2;
	unsigned short pio_program_instructions[] = {
		/* 00 */ (unsigned short)pio_encode_pull(false, true),
		/* 01 */ (unsigned short)pio_encode_wait_gpio(false, config.vsync_gpio),
		/* 02 */ (unsigned short)pio_encode_wait_gpio(true, config.pclk_gpio),
		/* 03 */ (unsigned short)pio_encode_set(pio_y, num_border_px - 1),
		/* 04 */ (unsigned short)pio_encode_wait_gpio(true, config.hsync_gpio), // border pixel y
		/* 05 */ (unsigned short)pio_encode_wait_gpio(false, config.hsync_gpio),
		/* 06 */ (unsigned short)pio_encode_jmp_y_dec(4),
		/* .wrap_target */
		/* 07 */ (unsigned short)pio_encode_mov(pio_x, pio_osr),
		/* 08 */ (unsigned short)pio_encode_wait_gpio(true, config.hsync_gpio),
		/* 09 */ (unsigned short)pio_encode_set(pio_y, num_border_px * num_pclk_per_px - 1),
		/* 10 */ (unsigned short)pio_encode_wait_gpio(true, config.pclk_gpio), // border pixel x
		/* 11 */ (unsigned short)pio_encode_wait_gpio(false, config.pclk_gpio),
		/* 12 */ (unsigned short)pio_encode_jmp_y_dec(10),
		/* 13 */ (unsigned short)pio_encode_wait_gpio(true, config.pclk_gpio),
		/* 14 */ (unsigned short)pio_encode_in(pio_pins, bus_bits),
		/* 15 */ (unsigned short)pio_encode_wait_gpio(false, config.pclk_gpio),
		/* 16 */ (unsigned short)pio_encode_jmp_x_dec(13),
		/* 17 */ (unsigned short)pio_encode_wait_gpio(false, config.hsync_gpio),
		/* .wrap */
  };
  ...
  pio_program_t pio_program;
	...
	if (*offset = pio_add_program(config.pio, &pio_program); static_cast<int>(*offset) < 0) {
		printf("PicoHM01B0::image_program_init:  pio_add_program failed with %d\n",static_cast<int>(*offset));
	  return static_cast<int>(*offset);
	}
	pio_sm_config c = pio_get_default_sm_config();
	sm_config_set_wrap(&c, *offset + 7, *offset+ pio_program.length - 1);

	pio_gpio_init(config.pio, config.d0_gpio);
	pio_sm_set_consecutive_pindirs(config.pio, config.pio_sm, config.d0_gpio, bus_bits, false);
	pio_gpio_init(config.pio, config.pclk_gpio);
	pio_sm_set_consecutive_pindirs(config.pio, config.pio_sm, config.pclk_gpio, 1, false);
	pio_gpio_init(config.pio, config.vsync_gpio);
	pio_sm_set_consecutive_pindirs(config.pio, config.pio_sm, config.vsync_gpio, 1, false);
	pio_gpio_init(config.pio, config.hsync_gpio);
	pio_sm_set_consecutive_pindirs(config.pio, config.pio_sm, config.hsync_gpio, 1, false);

	sm_config_set_in_pins(&c, config.d0_gpio);
	sm_config_set_in_shift(&c, true, true, 32);
	sm_config_set_fifo_join(&c, PIO_FIFO_JOIN_RX);
	if (int ret = pio_sm_init(config.pio, config.pio_sm, *offset, &c); ret < 0) {
		printf("PicoHM01B0::image_program_init:  pio_sm_init failed with %d\n",static_cast<int>(*offset));
	  return ret;
	}
	return 0;
}
...
int PicoHM01B0::begin(const PicoHM01B0_config &config)
{
	...
	if (int ret = image_program_init(&data_pio_offset); ret < 0) {
		printf("PicoHM01B0::begin:  image_program_init failed with %d\n", ret);
		if (config.mclk_gpio >= 0)
...
uint PicoHM01B0::start_capture(uint8_t *dest)
{
	...
	dma_channel_configure(dma_channel, &c, dest, &(config.pio->rxf[config.pio_sm]), image_buf_size, false);

	// prepare the PIO to restart
	pio_sm_restart(config.pio, config.pio_sm);
	pio_sm_clkdiv_restart(config.pio, config.pio_sm);
	pio_sm_put_blocking(config.pio, config.pio_sm, get_cols() * 8 - 1);
  pio_sm_exec(config.pio, config.pio_sm, pio_encode_jmp(data_pio_offset));
...
class MarquesHm01b0Streamer : public Hm01b0Streamer {
 private:
   PicoHM01B0 mq_hm01b0;
	 int dma_channel;
	 absolute_time_t capture_start;


 public:
  MarquesHm01b0Streamer() : dma_channel(-1) {};
  ~MarquesHm01b0Streamer() {};

  Hm01b0Err hm01b0Init(const struct Hm01b0Config* config, float fps) override {
		PicoHM01B0_config mq_config = {
			.i2c_dat_gpio = config->sda_pin,
			.i2c_clk_gpio = config->scl_pin,
			.vsync_gpio = config->vsync_pin,
			.hsync_gpio = config->hsync_pin,
			.d0_gpio = config->data_pin_base,
	    .pclk_gpio = config->pclk_pin,
			.mclk_gpio = config->mclk_pin,
			.mclk_freq = config->mclk_freq,
			.pio = config->pio ? pio1 : pio0,
			.pio_sm = config->pio_sm,
	    .bus_4bit = false,
	    .flip_horizontal = false,
			.flip_vertical = false
		};
		if (int ret = mq_hm01b0.begin(mq_config); ret < 0)
			return Hm01b0Err::HM01B0_ERR_INVALID_MODEL;
		mq_hm01b0.start_streaming(fps, false, true);
		return Hm01b0Err::HM01B0_ERR_OK;
	}

  void hm01b0Deinit() {
		mq_hm01b0.stop_streaming();
	}

  Hm01b0Err hm01b0ReadFrame(uint8_t* buffer, size_t length, int& dma_channel) override {
    capture_start = get_absolute_time();
		dma_channel = static_cast<int>(mq_hm01b0.start_capture(buffer));
    return Hm01b0Err::HM01B0_ERR_OK;
  }

  Hm01b0Err waitForFrameFinish(int dma_channel) override {
    mq_hm01b0.wait_for_frame();
    return Hm01b0Err::HM01B0_ERR_OK;
  }
...
Hm01b0Streamer* makeHm01b0Streamer() {
  return static_cast<Hm01b0Streamer*>(new MarquesHm01b0Streamer());
}