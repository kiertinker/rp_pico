...
class TcpConnectionImpl : public tcp_client::TcpConnection {
 ...
 private:
  ...
  err_t err_;
  mutex mtx_;
  mutex block_for_callback_mtx_;  // wait for data transmission ack.
  bool closed_;
  u16_t cb_written_ = 0;
  ...
  err_t connectComplete(err_t err) {
    ...
    err_ = err;
    mutex_exit(&block_for_callback_mtx_);
    return ERR_OK;
  }
  ...
  err_t sendData() {
        ...
        ? cb_send_buf : static_cast<unsigned short>(bytes_left);
    {
      ...
      err = tcp_write(socket_, buffer_pointer_, cb_to_send, 0);
    }
 ...
 public:
  ...
  tcp_client::TcpErr send(const unsigned char bytes[], unsigned int cb) override {
    {  // Scope the send mutex here so we can effectively return the resulting error or success.
      ...
      while (buffer_pointer_ != buffer_end_ && err_ == ERR_OK) {
        MutexLockAndWaitForRelease wait_lock(block_for_callback_mtx_);
        // We could have been unblocked by the err callback.
        if (err_ != ERR_OK  && err_ != ERR_MEM) break;
        int err_mem_retry = 10;
        // TODO:  This loop's condition logic could use some help.
        do {
          err_ = sendData();
          if (err_ != ERR_OK) {
            // Check if retriable (i.e. memory) error.  We will sleep and continue to retry once for these.
            if (err == ERR_MEM && err_mem_retry)
              sleep_ms(30);
            else
              // If we are here then the error is non-retriable or we are out of retries.
              // Release the waitlock to we can bail out without needing a callback trigger.
              wait_lock.release();
          }
        } while (err_mem_retry-- && err_ == ERR_MEM);
      }
