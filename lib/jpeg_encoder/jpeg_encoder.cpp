...
#define DCT1D(s0, s1, s2, s3, s4, s5, s6, s7) \
  ...
  s0 = t10 + t11; s1 = t7 + u1 + u4; s3 = t6 + u2 + u3; s4 = t10 - t11; s5 = t5 + u2 + u4; s7 = t4 + u1 + u3;
	...
	bool jpeg_encoder::jpg_open(int p_x_res, int p_y_res) {
      ...
      m_image_bpl = m_image_x;
      m_image_x_mcu = (m_image_x + m_mcu_x - 1) & (~(m_mcu_x - 1));
      m_image_y_mcu = (m_image_y + m_mcu_y - 1) & (~(m_mcu_y - 1));
      m_image_bpl_xlt = m_image_x;
      m_image_bpl_mcu = m_image_x_mcu;
	...
	void jpeg_encoder::load_quantized_coefficients()
	{
		int32* q = m_quantization_tables[0];
...
#define JPGE_PUT_BYTE(c) { *m_pOut_buf++ = (c); if (--m_out_buf_left == 0) flush_output_buffer(); }
	...
	void jpeg_encoder::code_coefficients_pass_two() {
      ...
      temp1 = temp2 = pSrc[0] - m_last_dc_val[0];
      m_last_dc_val[0] = pSrc[0];
	...
	void jpeg_encoder::code_block()
	{
		DCT2D(m_sample_array);
		load_quantized_coefficients();
		if (m_pass_num == 1)
			code_coefficients_pass_one();
		else
			code_coefficients_pass_two();
	}

	void jpeg_encoder::process_mcu_row() {
      for (int i = 0; i < m_mcus_per_row; i++) load_block_8_8_grey(i); code_block(0);
	}
