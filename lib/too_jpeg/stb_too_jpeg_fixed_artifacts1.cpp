...
inline fixed_point fixed_to_int(fixed_point fixed) {
  return (fixed > -(1 << FIXED_POINT_FRACTIONAL_BITS) && fixed < 1 << FIXED_POINT_FRACTIONAL_BITS) ?
      0 : (fixed >> FIXED_POINT_FRACTIONAL_BITS);
}
inline fixed_point fixed_to_int_rounded(fixed_point fixed) {
  return fixed_to_int(fixed + ((fixed >= 0) ?
       (1 << (FIXED_POINT_FRACTIONAL_BITS - 1)) : -(1 << (FIXED_POINT_FRACTIONAL_BITS - 1))));
}
inline fixed_point fixed_scaled_to_int(fixed_point fixed) {
  return (fixed > -(1 << SCALED_POINT_SCALED_FRACTIONAL_BITS) && fixed < 1 << SCALED_POINT_SCALED_FRACTIONAL_BITS) ?
      0 : (fixed >> SCALED_POINT_SCALED_FRACTIONAL_BITS);
}
inline int fixed_scaled_to_int_rounded(fixed_point fixed) {
  return fixed_scaled_to_int(fixed + ((fixed >= 0) ?
       (1 << (SCALED_POINT_SCALED_FRACTIONAL_BITS - 1)) : -(1 << (SCALED_POINT_SCALED_FRACTIONAL_BITS - 1))));
}

inline fixed_point fx_mul(fixed_point x, fixed_point y) {
  int64_t result = static_cast<int64_t>(x) * static_cast<int64_t>(y);
  fixed_point ret = fixed_to_int_rounded(result);
  if (result > (static_cast<int64_t>(INT32_MAX) << FIXED_POINT_FRACTIONAL_BITS) || result < (static_cast<int64_t>(INT32_MIN) << FIXED_POINT_FRACTIONAL_BITS)) {
...
inline fixed_point fx_scaled_mul(fixed_point x, fixed_point y) {
  int64_t result = static_cast<int64_t>(x) * static_cast<int64_t>(y);
  fixed_point ret = fixed_scaled_to_int_rounded(result);
  if (result > (static_cast<int64_t>(INT32_MAX) << SCALED_POINT_SCALED_FRACTIONAL_BITS) || result < (static_cast<int64_t>(INT32_MIN) << SCALED_POINT_SCALED_FRACTIONAL_BITS)) {
    printf("!!!!!!!!!!!!!!!! fx_mul overflow/underflow result: %lld  return = %d !!!!!!!!\n", result, ret);
  }
  return ret;
}
...
struct BitWriter
{
  ...
  too_jpeg::WRITE_ONE_BYTE output;
  // initialize writer
  explicit BitWriter(too_jpeg::WRITE_ONE_BYTE output_) : output(output_) {}

  // store the most recently encoded bits that are not written yet
  struct BitBuffer
  {
    int data    = 0; // actually only at most 24 bits are used
...
void DCT(fixed_point block[8*8], uint8_t stride) // stride must be 1 (=horizontal) or 8 (=vertical)
{
  const auto SqrtHalfSqrt = float_to_fixed(1.306562965f); //    sqrt((2 + sqrt(2)) / 2) = cos(pi * 1 / 8) * sqrt(2)
  const auto InvSqrt      = float_to_fixed(0.707106781f); // 1 / sqrt(2)                = cos(pi * 2 / 8)
  const auto HalfSqrtSqrt = float_to_fixed(0.382683432f); //     sqrt(2 - sqrt(2)) / 2  = cos(pi * 3 / 8)
  const auto InvSqrtSqrt  = float_to_fixed(0.541196100f); // 1 / sqrt(2 - sqrt(2))      = cos(pi * 3 / 8) * sqrt(2)
...
int16_t encodeBlock(BitWriter& writer, fixed_point block[8][8], const fixed_point scaled[8*8], int16_t lastDC,
                    const BitCode huffmanDC[256], const BitCode huffmanAC[256], const BitCode* codewords)
{
  ...
  auto block64 = (fixed_point*) block;
  ...
  for (auto i = 0; i < 8*8; i++)
    block64[i] = fx_scaled_mul(block64[i], scaled[i]);

  // encode DC (the first coefficient is the "average color" of the 8x8 block)
  auto DC = fixed_to_int_rounded(block64[0]); // C++11's nearbyint() achieves a similar effect
  ...
  for (auto i = 1; i < 8*8; i++) // start at 1 because block64[0]=DC was already processed
  {
    ...
    quantized[i] = static_cast<int16_t>(fixed_to_int_rounded(value)); // C++11's nearbyint() achieves a similar effect
...
void initConstants(unsigned char quality) {
  // quality level must be in 1 ... 100
  auto qual = clamp<uint16_t>(quality, 1, 100);
  ...
  for (auto i = 0; i < 8*8; i++)
  {
    ...
    scaledLuminance  [ZigZagInv[i]] = scaled_float_to_fixed(factor / quantLuminance  [i]);
  }
...
bool writeJpeg(too_jpeg::WRITE_ONE_BYTE output, const void* pixels_, unsigned short width, unsigned short height,
               bool isRGB, unsigned char quality_, bool downsample, const char* comment)
{
  ...
  bitWriter.addMarker(0xDB, 2 + 1 * (1 + 8*8)); // length: 65 bytes per table + 2 bytes for this length field
  ...
  bitWriter.addMarker(0xC0, 2+6+3*1); // length: 6 bytes general info + 3 per channel + 2 bytes for this length field
  ...
  bitWriter << 1;       // 1 component (grayscale, Y only) or 3 components (Y,Cb,Cr)
    bitWriter <<  1                // component ID (Y=1, Cb=2, Cr=3)
    // bitmasks for sampling: highest 4 bits: horizontal, lowest 4 bits: vertical
              ...
              << 0; // use quantization table 0 for Y, table 1 for Cb and Cr

  // ////////////////////////////////////////
  // Huffman tables
  // DHT marker - define Huffman tables
  bitWriter.addMarker(0xC4, (2+208));
  ...
  bitWriter.addMarker(0xDA, 2+1+2*1+3); // 2 bytes for the length field, 1 byte for number of components,
  ...
  bitWriter << 1;
    ...
    bitWriter << 1 << 0x00; // Y: tables 0 for DC and AC; Cb + Cr: tables 1 for DC and AC
        ...
        for (auto blockX = 0; blockX < mcuSize; blockX += 8)
        {
          ...
          for (auto deltaY = 0; deltaY < 8; deltaY++)
          {
            ...
            for (auto deltaX = 0; deltaX < 8; deltaX++)
            {
              ...
              Y[deltaY][deltaX] = float_to_fixed(pixels[pixelPos] - 128.f);
            }
          }
...
class StbTooJpeg : public too_jpeg::ITooJpeg {
  too_jpeg::WRITE_ONE_BYTE output_;
  unsigned char quality_;

 public:
  StbTooJpeg(too_jpeg::WRITE_ONE_BYTE output, unsigned char quality) : output_(output), quality_(quality) {
    initConstants(quality);
  }
  virtual ~StbTooJpeg() {}
  bool writeJpeg(
      const void* pixels, unsigned short width, unsigned short height, const char* comment = nullptr) override {
    return TooJpeg::writeJpeg(output_, pixels, width, height, false, quality_, false, comment);
  }
};

too_jpeg::ITooJpeg* too_jpeg::getTooJpeg(too_jpeg::WRITE_ONE_BYTE output, unsigned char quality) {
  return dynamic_cast<too_jpeg::ITooJpeg*>(new StbTooJpeg(output, quality));
}
