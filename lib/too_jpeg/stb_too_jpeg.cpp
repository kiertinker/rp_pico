...
inline void getFillBuffer() {
  if (g_packet_header != nullptr)
   *g_packet_header = g_out_buffer_end - (reinterpret_cast<unsigned char*>(g_packet_header) + 4);
  g_packet_header = reinterpret_cast<size_t*>(g_getFillBuffer(g_out_buffer_end));
...
struct BitWriter
{
  ...
  BitWriter& operator<<(const BitCode& data)
  {
    ...
    while (buffer.numBits >= 8)
    {
      ...
      buffer.numBits -= 8;
      unsigned char oneByte = uint8_t(buffer.data >> buffer.numBits);
      output(oneByte);
...
too_jpeg::WRITE_JPEG_RESULT writeJpeg(const void* pixels_, unsigned short width, unsigned short height,
    bool isRGB, unsigned char quality_, bool downsample, std::string_view comment)
{
  static unsigned char* g_out_buffer_end = nullptr;
...
class StbTooJpeg : public too_jpeg::ITooJpeg {
 ...
 public:
  StbTooJpeg(too_jpeg::GET_BUFFER get_buffer_func, size_t buffer_size, unsigned short width, unsigned short height, unsigned char quality)
  ...
  virtual ~StbTooJpeg() {}
  too_jpeg::WRITE_JPEG_RESULT writeJpeg(
      const void* pixels, std::string_view comment = "") override {
    return TooJpeg::writeJpeg(pixels, width_, height_, false, quality_, false, comment);
  }
};
...
too_jpeg::ITooJpeg* too_jpeg::getTooJpeg(
  ...
  return dynamic_cast<too_jpeg::ITooJpeg*>(new StbTooJpeg(get_buffer_func, buffer_size, width, height, quality));
}
