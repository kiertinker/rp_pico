...
namespace too_jpeg {
  // function that returns a buffer from external consumer
  using GET_BUFFER = unsigned char* (*)(unsigned char* end);

  enum class WRITE_JPEG_RESULT {
    WJR_OK,
    WJR_NO_BUFFER,
    WJR_ERROR
  };
...
class ITooJpeg {
 public:
  ITooJpeg() {}
  virtual ~ITooJpeg() {}
  virtual WRITE_JPEG_RESULT writeJpeg(const void* pixels, std::string_view comment = "");
};

ITooJpeg* getTooJpeg(GET_BUFFER, size_t buffer_size, unsigned short width, unsigned short height, unsigned char quality);
